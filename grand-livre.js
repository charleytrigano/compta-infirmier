...
window.initGrandLivre = function() {
    var container = document.getElementById('grandlivre-container') || document.querySelector('.card, .container');

    if (!container) {
        var elements = document.querySelectorAll('div, section');
        elements.forEach(function(el) {
            if (el.textContent.includes('Grand Livre') || el.textContent.includes('Grand-Livre')) {
                container = el;
            }
        });
    }
...
// Fonction d'analyse comptable identique au Journal (Priorité Catégorie)
...
// Génération de toutes les écritures brutes
...
// Affichage et rendu du Grand Livre
window.afficherGrandLivre = function(container) {
    ...
    // Tri des numéros de compte dans l'ordre comptable (4xx, 5xx, 6xx, 7xx)
